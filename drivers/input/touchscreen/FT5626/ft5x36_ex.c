...
bool ft_gesture_enable;
extern bool ft_gesture_enable_dtsi;
//bool gesture_enable;
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	if(uc_tp_fm_TP_ID == 0 || uc_tp_fm_TP_ID != FT5626_VENDOR_LAIBAO){
	...
	case FT5626_VENDOR_LAIBAO:
		fw_len = sizeof(FT5626_FIRMWARE0x59_LAIBAO);
		...
		pbt_buf = FT5626_FIRMWARE0x59_LAIBAO;
		/*call the upgrade function*/
		i_ret = fts_5822_ctpm_fw_upgrade(client, pbt_buf, sizeof(FT5626_FIRMWARE0x59_LAIBAO));
		if (i_ret != 0) 
		{
...
u8 fts_ctpm_get_i_file_ver(struct i2c_client *client)
{
	...
	case FT5626_VENDOR_LAIBAO:
	 {
		ui_sz = sizeof(FT5626_VENDOR_LAIBAO);
		 if (ui_sz > 2)
		 return FT5626_FIRMWARE0x59_LAIBAO[0x10A];
		break;
	}
	case FT5X36_VENDOR_JTOUCH:
...
//added by chenchen for focaltech FT6336 20140902 begin
	case IC_FT6x36:
		...
		break;	
	case IC_FT5626:
		upgrade_info->delay_55 = FT5626_UPGRADE_55_DELAY;
		upgrade_info->delay_aa = FT5626_UPGRADE_AA_DELAY;
		upgrade_info->upgrade_id_1 = FT5626_UPGRADE_ID_1;
		upgrade_info->upgrade_id_2 = FT5626_UPGRADE_ID_2;
		upgrade_info->delay_readid = FT5626_UPGRADE_READID_DELAY;
		upgrade_info->delay_earse_flash = FT5626_UPGRADE_EARSE_DELAY;
		break;
...
//modify by pangle for fw upgrade at 20150501 begin
    if(ft5x36_ts->pdata->family_id == 8)
    {
	...
	for (i = 0; i < hw_rest_loop; i++) {
		...
		if(!hw_flag)
		{
			...
			//msleep(i*5+10);
//					//modify  for delay by pangle at 2014.09.15
			if(ft5x36_ts->pdata->family_id == 8)
    		{
				msleep(i+4);
			}
...
//modify by pangle for fw upgrade at 20150501 end
	...
	for(j=0;j<2;j++)
	{
		fts_i2c_Write(client, packet_buf, sizeof(packet_buf));
		msleep(5);
		i_ret=fts_i2c_Read(client, packet_buf, 0, 
				pProjectCode, 2);
		DBG("project code = 0x%x i_ret=%d\n", pProjectCode[0],i_ret);	// modify by pangle at 20140923
		if(pProjectCode[0]!=0)
		{
		  break;
		}
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		msleep(upgradeinfo.delay_aa+i+2);
		fts_write_reg(client, 0xfc, FT_UPGRADE_55);
